...
impl MyImageManager {
    ...
    /// 获取Image的Manifest
    pub async fn manifests(&mut self, refe: &Reference<'_>, platform: Option<Platform>) -> Result<(Manifest, String)> {
        let accepts = &[
        ...
        let response = self.request_manifest(refe, accepts).await?;
        let result = match response.manifest() {
            ...
            ManifestResponseEnum::ManifestList(manifest_list) => {
                ...
                self.select_manifest(refe, &manifest_list, pf).await?
            }
        };
        Ok(result)
    }

    pub async fn select_manifest(&mut self, refe: &Reference<'_>, manifest_list: &ManifestList, platform: Platform) -> Result<(Manifest, String)> {
        let digest =
        ...
        let response = self.request_manifest(&reference, &accepts).await?;
    ...
    pub async fn request_manifest(&mut self, refe: &Reference<'_>, accepts: &[RegContentType]) -> Result<ManifestResponse> {
        ...
        let response_body = response.string_body().await?;
        ManifestResponse::from(content_type, response_body)
    }

    /// Image blobs是否存在
    pub async fn blobs_exited(&mut self, name: &str, blob_digest: &RegDigest) -> Result<bool> {
        let path = format!("/v2/{}/blobs/{}", name, blob_digest.digest);
        let scope = Some(name);
        let request = ClientRequest::new_head_request(&path, scope, TokenType::Pull);
        let response = self.reg_client.simple_request::<u8>(request).await?;
        exited(&response)
    }

    pub async fn config_blob<T: ConfigBlob + DeserializeOwned>(&mut self, name: &str, blob_digest: &str) -> Result<(T, String)> {
    ...
    pub async fn layer_blob_download(&mut self, name: &str, blob_digest: &RegDigest, layer_size: Option<u64>) -> Result<RegDownloader> {
        ...
        let downloader = self.reg_client.download(&url_path, blob_config, name, layer_size).await?;
        Ok(downloader)
    }

    /// 上传layer类型的blob文件
    pub async fn layer_blob_upload(&mut self, name: &str, blob_digest: &RegDigest, file_local_path: &str) -> Result<RegUploader> {
        ...
        if self.blobs_exited(name, blob_digest).await? {
        ...
        let mut location_url = self.layer_blob_upload_ready(name).await?;
        ...
        let reg_uploader = self.reg_client.upload(location_url.to_string(), blob_config, name, &file_path).await?;
        Ok(reg_uploader)
    }
    ...
    pub async fn put_manifest(&mut self, refe: &Reference<'_>, manifest: Manifest) -> Result<(StatusCode, String)> {
        let path = format!("/v2/{}/manifests/{}", refe.image_name, refe.reference);
        let scope = Some(refe.image_name);
        let response = match manifest {
            Manifest::OciV1(oci_manifest) => {
                ...
                self.reg_client.simple_request::<OciManifest>(request).await?
            }
            Manifest::DockerV2S2(docker_v2s2_manifest) => {
                ...
                self.reg_client.simple_request::<DockerManifest>(request).await?
            }
        };
