...
impl Registry {
    pub async fn open(use_https: bool, host: &str, reg_cteate_info: RegistryCreateInfo) -> Result<Registry> {
        let host = match host {
...
impl MyImageManager {
    ...
    /// 获取Image的Manifest
    pub async fn manifests(&mut self, refe: &Reference, platform: Option<Platform>) -> Result<(Manifest, String)> {
        let accepts = &[
    ...
    pub async fn select_manifest(
        ...
        let reference = Reference {
            image_name: refe.image_name.clone(),
            reference: digest,
        };
